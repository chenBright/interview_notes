...
`ulimit` 用于限制 shell 启动进程所占用的资源，支持以下各种类型的限制：**所创建的内核文件的大小**、**进程数据块的大小**、**Shell进程创建文件的大小**、**内存锁住的大小**、**常驻内存集的大小**、**打开文件描述符的数量**、**分配堆栈的最大大小**、**CPU时间**、**单个用户的最大线程数**、**Shell进程所能使用的最大虚拟内存**。同时，它支持**硬资源**和**软资源**的限制。
